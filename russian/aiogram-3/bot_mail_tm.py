#!/usr/bin/env python3
"""
Mail.tm Telegram Bot (aiogram 3.x)
Provider: Mail.tm | API: https://api.mail.tm
...
BOT_TOKEN = os.environ.get("BOT_TOKEN_MAIL_TM", "YOUR_BOT_TOKEN")
...
async def cmd_start(m):
    await m.answer("*Mail.tm Бот*\\nREST API с созданием аккаунтов\\n\\n*Возможности:*\\nДомены, аккаунты, входящие, чтение сообщений\\n\\n*Как пользоваться:*\\n1. Нажмите 'Новая почта'\\n2. Скопируйте адрес\\n3. Используйте для регистрации\\n4. Нажмите 'Входящие'\\n5. Новые помечены эмодзи", reply_markup=make_kb())
...
async def cmd_info(m):
    await m.answer('*Mail.tm — Информация*\\n\\n*Сервис:* Mail.tm\\n*Описание:* REST API с созданием аккаунтов\\n*Возможности:* Домены, аккаунты, входящие, чтение сообщений\\n*API:* `https://api.mail.tm`\\n*Сайт:* https://mail.tm\\n*Код:* https://github.com/cpner/temp-email-api-checker/blob/main/ru/aiogram-3/bot_mail_tm.py\\n*Автор:* Владислав Софронов (cpner)\\n*Лицензия:* MIT', reply_markup=make_kb())

@dp.message(F.text == "/help")
async def cmd_help(m):
    await m.answer('*Mail.tm — Команды*\\n\\n/start — Главное меню\\n/new — Создать почту\\n/inbox — Проверить письма\\n/set — Установить почту\\n/info — Информация\\n/help — Эта справка\\n\\n*Кнопки:*\\n📧 Новая почта — Создать адрес\\n📥 Входящие — Проверить письма\\nℹ️ Инфо — О боте\\n🔗 Код — Ссылка на GitHub\\n❓ Помощь — Инструкция', reply_markup=make_kb())
...
async def cb_help(call):
    await bot.send_message(call.message.chat.id, '*Mail.tm — Команды*\\n\\n/start — Главное меню\\n/new — Создать почту\\n/inbox — Проверить письма\\n/set — Установить почту\\n/info — Информация\\n/help — Эта справка\\n\\n*Кнопки:*\\n📧 Новая почта — Создать адрес\\n📥 Входящие — Проверить письма\\nℹ️ Инфо — О боте\\n🔗 Код — Ссылка на GitHub\\n❓ Помощь — Инструкция', reply_markup=make_kb())
