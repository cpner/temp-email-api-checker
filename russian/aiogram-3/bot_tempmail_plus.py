...
{ap}def cmd_set(m):
    ...
    if len(p)<2: return {aw}bot.send_message(m.chat.id,"/set email@domain.com")
    s=get_session(m.chat.id); s.addr,s.seen=p[1].strip(),set()
    {aw}bot.send_message(m.chat.id,f"✅ Мониторинг: `{s.addr}`")
...
{ap}def cmd_inbox(m):
    ...
    r=api_get(params={"email":s.addr}); mails=r.get("mail",[]); stats["checked"]+=1
    if not mails: return {aw}bot.send_message(c,"📭 Пусто")
    t=f"*{len(mails)} писем*\n\n"
    for x in mails[:15]:
