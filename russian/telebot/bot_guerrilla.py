...
Исходный код: https://github.com/cpner/temp-email-api-checker/blob/main/russian/telebot/bot_guerrilla.py
Лицензия: MIT
"""
...
# Текстовые сообщения
SOURCE_URL = "https://github.com/cpner/temp-email-api-checker/blob/main/russian/telebot/bot_guerrilla.py"

START_TEXT = """*Guerrilla Mail Бот*
...
HELP_TEXT = """*Guerrilla Mail — Команды*
...
def handle_new(user_id: int, session: UserSession) -> str:
    ...
    if "email_addr" in r:
        session.addr = r["email_addr"]; session.token = r.get("sid_token"); session.seen = set(); session.ts = time.time()
        stats["created"] += 1
        return "✅ Создано: `" + r["email_addr"] + "`"
    return "❌ Ошибка. Попробуйте /new"

def handle_inbox(user_id: int, session: UserSession) -> str:
    """Проверить входящие."""
    if not session.token: return "❌ Сначала /new"
    r = api_get(params={{"f":"check_email","sid_token":session.token,"seq":0}})
    msgs = r.get("list",[]); stats["checked"] += 1
    if not msgs: return "📭 Пусто"
...
def handle_set_message(text: str, session: UserSession) -> str:
    ...
    if len(p)<2: return "Использование: /set <имя>"
    r = api_get(params={{"f":"set_email_user","sid_token":session.token or "","email_user":p[1].strip()}})
    if "email_addr" in r: session.addr = r["email_addr"]; return "✅ Почта: `" + r["email_addr"] + "`"
    return "❌ Ошибка"
