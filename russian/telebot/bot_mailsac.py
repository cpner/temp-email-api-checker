...
Провайдер: MailSac | API: https://mailsac.com/api
...
if not BOT_TOKEN or BOT_TOKEN == "YOUR_BOT_TOKEN":
    logger.error("Не задан BOT_TOKEN! Установите переменную BOT_TOKEN_MAILSAC")
    sys.exit(1)
...
def callback_handler(call: types.CallbackQuery) -> None:
    ...
    try:
        if action == "new":
        bot.send_message(cid, f"Посетите https://mailsac.com/api")
        elif action == "inbox":
        bot.send_message(cid, f"Посетите https://mailsac.com/api")
        elif action == "info":
            s = get_session(cid)
