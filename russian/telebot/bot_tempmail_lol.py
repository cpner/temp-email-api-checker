...
# Текстовые сообщения
SOURCE_URL = "https://github.com/cpner/temp-email-api-checker/blob/main/russian/telebot/bot_tempmail_lol.py"

START_TEXT = """*TempMail.lol Бот*
Генерация почты + токен

*Возможности:*
Генерация, токен, проверка
...
INFO_TEXT = """*TempMail.lol — Инфо*

*Сервис:* TempMail.lol
*Описание:* Генерация почты + токен
*Возможности:* Генерация, токен, проверка
*API:* `https://api.tempmail.lol`
...
HELP_TEXT = """*TempMail.lol — Команды*
...
def handle_new(user_id: int, session: UserSession) -> str:
    ...
    r = api_get("/generate")
    if "address" in r:
        session.addr = r["address"]; session.token = r.get("token"); session.seen = set()
        stats["created"] += 1
        return "✅ Создано: `" + r["address"] + "`"
    return "❌ Ошибка"

def handle_inbox(user_id: int, session: UserSession) -> str:
    """Проверить входящие."""
    if not session.token: return "❌ Сначала /new"
    r = api_get("/auth/"+session.token); emails = r.get("email",[]); stats["checked"] += 1
    if not emails: return "📭 Пусто"
    t = str(len(emails)) + " писем:\n"
    for e in emails[:15]:
        eid = e.get("id","?"); n = "🆕 " if eid not in session.seen else ""
        session.seen.add(eid)
        t += n + "`" + eid + "` | " + e.get("from","?") + " | " + e.get("subject","-") + "\n"
    return t
