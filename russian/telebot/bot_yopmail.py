...
def cmd_start(m):
    bot.send_message(m.chat.id, "*YOPmail Бот*\\nМногодоменная одноразовая почта\\n\\n*Возможности:*\\nПроверка входящих, список доменов\\n\\n*Как пользоваться:*\\n1. Нажмите 'Новая почта'\\n2. Скопируйте адрес\\n3. Используйте для регистрации\\n4. Нажмите 'Входящие'\\n5. Новые помечены эмодзи", reply_markup=make_kb())
...
def cmd_info(m):
    bot.send_message(m.chat.id, '*YOPmail — Информация*\\n\\n*Сервис:* YOPmail\\n*Описание:* Многодоменная одноразовая почта\\n*Возможности:* Проверка входящих, список доменов\\n*API:* `https://yopmail.com`\\n*Сайт:* https://yopmail.com\\n*Код:* https://github.com/cpner/temp-email-api-checker/blob/main/ru/telebot/bot_yopmail.py\\n*Автор:* Владислав Софронов (cpner)\\n*Лицензия:* MIT', reply_markup=make_kb())

@bot.message_handler(commands=["help"])
def cmd_help(m):
    bot.send_message(m.chat.id, '*YOPmail — Команды*\\n\\n/start — Главное меню\\n/new — Создать почту\\n/inbox — Проверить письма\\n/set — Установить почту\\n/info — Информация\\n/help — Эта справка\\n\\n*Кнопки:*\\n📧 Новая почта — Создать адрес\\n📥 Входящие — Проверить письма\\nℹ️ Инфо — О боте\\n🔗 Код — Ссылка на GitHub\\n❓ Помощь — Инструкция', reply_markup=make_kb())
...
def cb(call):
    ...
    try:
        ...
        if a == "new":
            ...
            bot.send_message(c, '*YOPmail — Команды*\\n\\n/start — Главное меню\\n/new — Создать почту\\n/inbox — Проверить письма\\n/set — Установить почту\\n/info — Информация\\n/help — Эта справка\\n\\n*Кнопки:*\\n📧 Новая почта — Создать адрес\\n📥 Входящие — Проверить письма\\nℹ️ Инфо — О боте\\n🔗 Код — Ссылка на GitHub\\n❓ Помощь — Инструкция', reply_markup=make_kb())
    except Exception as e:
