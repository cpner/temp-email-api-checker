#!/usr/bin/env python3
"""
Burner.kiwi Telegram Bot (aiogram 2.x)
...
async def cmd_start(m):
    await m.answer("*Burner.kiwi Бот*\\nБыстрая одноразовая почта на 24 часа\\n\\n*Возможности:*\\nБыстрое создание, истечение 24ч\\n\\n*Как пользоваться:*\\n1. Нажмите 'Новая почта'\\n2. Скопируйте адрес\\n3. Используйте для регистрации\\n4. Нажмите 'Входящие'\\n5. Новые помечены эмодзи", reply_markup=make_kb())
...
async def cmd_info(m):
    await m.answer('*Burner.kiwi — Информация*\\n\\n*Сервис:* Burner.kiwi\\n*Описание:* Быстрая одноразовая почта на 24 часа\\n*Возможности:* Быстрое создание, истечение 24ч\\n*API:* `https://burner.kiwi`\\n*Сайт:* https://burner.kiwi\\n*Код:* https://github.com/cpner/temp-email-api-checker/blob/main/ru/aiogram-2/bot_burner_kiwi.py\\n*Автор:* Владислав Софронов (cpner)\\n*Лицензия:* MIT', reply_markup=make_kb())

@dp.message_handler(commands=["help"])
async def cmd_help(m):
    await m.answer('*Burner.kiwi — Команды*\\n\\n/start — Главное меню\\n/new — Создать почту\\n/inbox — Проверить письма\\n/set — Установить почту\\n/info — Информация\\n/help — Эта справка\\n\\n*Кнопки:*\\n📧 Новая почта — Создать адрес\\n📥 Входящие — Проверить письма\\nℹ️ Инфо — О боте\\n🔗 Код — Ссылка на GitHub\\n❓ Помощь — Инструкция', reply_markup=make_kb())
...
async def cb(call):
    ...
    try:
        ...
        if a == "new":
            ...
            await bot.send_message(c, '*Burner.kiwi — Команды*\\n\\n/start — Главное меню\\n/new — Создать почту\\n/inbox — Проверить письма\\n/set — Установить почту\\n/info — Информация\\n/help — Эта справка\\n\\n*Кнопки:*\\n📧 Новая почта — Создать адрес\\n📥 Входящие — Проверить письма\\nℹ️ Инфо — О боте\\n🔗 Код — Ссылка на GitHub\\n❓ Помощь — Инструкция', reply_markup=make_kb())
    except Exception as e:
