#!/usr/bin/env python3
"""
TempMail.plus — ProtonMail

Provider: TempMail.plus | Endpoint: api/mails?email=...@protonmail.com
Docs: tempmail.plus
"""
import telebot, requests, random, string, time, os

BOT_TOKEN = os.environ.get("BOT_TOKEN_TP_PROTONMAIL_COM", "YOUR_TOKEN")
bot = telebot.TeleBot(BOT_TOKEN)
BASE = "https://tempmail.plus/api/mails"
SVC = "TempMail.plus ProtonMail"
sess = {}
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("New Email", callback_data="tp13_new"))
    kb.add(types.InlineKeyboardButton("Inbox", callback_data="tp13_inbox"))
    kb.add(types.InlineKeyboardButton("Info", callback_data="tp13_info"))
    kb.add(types.InlineKeyboardButton("Help", callback_data="tp13_help"))
    bot.send_message(m.chat.id, "*TempMail.plus — ProtonMail*\n\n\n/new — Create\n/inbox — Check\n/info — Info\n/help — Help", parse_mode="Markdown", reply_markup=kb)

@bot.message_handler(commands=["set"])
def cmd_set(m):
    p=m.text.split(maxsplit=1)
    if len(p)<2: return bot.send_message(m.chat.id,"/set email@protonmail.com")
    s=gs(m.chat.id); s["addr"]=p[1].strip(); s["seen"]=set(); bot.send_message(m.chat.id,f"Monitoring: `{s['addr']}`",parse_mode="Markdown")

@bot.message_handler(commands=["inbox"])
def cmd_inbox(m):
    c=m.chat.id; s=gs(c)
    if not s.get("addr"): return bot.send_message(c,"/set email@protonmail.com")
...
def cmd_info(m): s=gs(m.chat.id); bot.send_message(m.chat.id,f"{s.get('addr','—')} | Provider: protonmail.com")

@bot.message_handler(commands=["help"])
def cmd_help(m): bot.send_message(m.chat.id,"/set <email> — Set\n/inbox — Check")

def cb_new(c,call): bot.send_message(c,"/set email@protonmail.com")
def cb_inbox(c,call):
    s=gs(c)
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("tp13_"))
def cb(call):
    c = call.message.chat.id
    a = call.data.replace("tp13_","")
