#!/usr/bin/env python3
"""
Guerrilla Spam4.me Telegram Bot
...
BOT_TOKEN = os.environ.get("BOT_TOKEN_GS4", "YOUR_TOKEN")
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("📧 Новая", callback_data="gs4_new"))
    kb.add(types.InlineKeyboardButton("📥 Письма", callback_data="gs4_inbox"))
    kb.add(types.InlineKeyboardButton("🔑 Ключ", callback_data="gs4_key"))
    kb.add(types.InlineKeyboardButton("📋 Данные", callback_data="gs4_info"))
    kb.add(types.InlineKeyboardButton("❓ Помощь", callback_data="gs4_help"))
    text = (
        "📧 *Guerrilla Spam4.me Bot*\n"
        "Альтернативный домен\n\n"
        "/new — Создать почту\n"
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("gs4_"))
def cb(call):
    cid = call.message.chat.id
    act = call.data.replace("gs4_", "")
...
if __name__ == "__main__":
    print("[Guerrilla Spam4.me Bot] Running...")
