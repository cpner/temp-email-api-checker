...
BOT_TOKEN = os.environ.get("BOT_TOKEN_EOD", "YOUR_TOKEN")
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("📧 Новая", callback_data="eod_new"))
    kb.add(types.InlineKeyboardButton("📥 Письма", callback_data="eod_inbox"))
    kb.add(types.InlineKeyboardButton("🔑 Ключ", callback_data="eod_key"))
    kb.add(types.InlineKeyboardButton("📋 Данные", callback_data="eod_info"))
    kb.add(types.InlineKeyboardButton("❓ Помощь", callback_data="eod_help"))
    text = (
        "📧 *EmailOnDeck Bot*\n"
        "Быстрая\n\n"
        "/new — Создать почту\n"
