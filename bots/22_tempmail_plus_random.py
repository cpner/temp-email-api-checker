#!/usr/bin/env python3
"""
TempMail.plus — Random Domain

Provider: TempMail.plus | Endpoint: api/mails?email=...@random.com
Docs: tempmail.plus
"""
import telebot, requests, random, string, time, os

BOT_TOKEN = os.environ.get("BOT_TOKEN_TP_RANDOM", "YOUR_TOKEN")
bot = telebot.TeleBot(BOT_TOKEN)
BASE = "https://tempmail.plus/api/mails"
SVC = "TempMail.plus Random"
sess = {}
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("New Email", callback_data="tpr_new"))
    kb.add(types.InlineKeyboardButton("Inbox", callback_data="tpr_inbox"))
    kb.add(types.InlineKeyboardButton("Info", callback_data="tpr_info"))
    kb.add(types.InlineKeyboardButton("Help", callback_data="tpr_help"))
    bot.send_message(m.chat.id, "*TempMail.plus — Random*\n\n\n/new — Create\n/inbox — Check\n/info — Info\n/help — Help", parse_mode="Markdown", reply_markup=kb)

@bot.message_handler(commands=["set"])
def cmd_set(m):
    p=m.text.split(maxsplit=1)
    if len(p)<2: return bot.send_message(m.chat.id,"/set email@domain")
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("tpr_"))
def cb(call):
    c = call.message.chat.id
    a = call.data.replace("tpr_","")
