#!/usr/bin/env python3
"""
TempMail.io Telegram Bot
API v1
API: https://temp-mail.io/api/v1
"""
import telebot
...
BOT_TOKEN = os.environ.get("BOT_TOKEN_TMIO", "YOUR_TOKEN")
bot = telebot.TeleBot(BOT_TOKEN)
BASE = "https://temp-mail.io/api/v1"
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("📧 Новая", callback_data="tio_new"))
    kb.add(types.InlineKeyboardButton("📥 Письма", callback_data="tio_inbox"))
    kb.add(types.InlineKeyboardButton("🔑 Ключ", callback_data="tio_key"))
    kb.add(types.InlineKeyboardButton("📋 Данные", callback_data="tio_info"))
    kb.add(types.InlineKeyboardButton("❓ Помощь", callback_data="tio_help"))
    text = (
        "📧 *TempMail.io Bot*\n"
        "API v1\n\n"
        "/new — Создать почту\n"
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("tio_"))
def cb(call):
    cid = call.message.chat.id
    act = call.data.replace("tio_", "")
