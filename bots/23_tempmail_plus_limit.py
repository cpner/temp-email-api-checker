#!/usr/bin/env python3
"""
TempMail.plus — Limit Parameter

Provider: TempMail.plus | Endpoint: api/mails?limit=N
Docs: tempmail.plus
"""
import telebot, requests, random, string, time, os

BOT_TOKEN = os.environ.get("BOT_TOKEN_TP_LIMIT", "YOUR_TOKEN")
bot = telebot.TeleBot(BOT_TOKEN)
BASE = "https://tempmail.plus/api/mails"
SVC = "TempMail.plus Limit"
sess = {}
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("New Email", callback_data="tpl_new"))
    kb.add(types.InlineKeyboardButton("Inbox", callback_data="tpl_inbox"))
    kb.add(types.InlineKeyboardButton("Info", callback_data="tpl_info"))
    kb.add(types.InlineKeyboardButton("Help", callback_data="tpl_help"))
    bot.send_message(m.chat.id, "*TempMail.plus — Limit*\n\n\n/new — Create\n/inbox — Check\n/info — Info\n/help — Help", parse_mode="Markdown", reply_markup=kb)
...
def cmd_inbox(m):
    ...
    r=g(p={"email":s["addr"],"limit":25}); mails=r.get("mail",[])
    if not mails: return bot.send_message(c,"Empty.")
    t=f"*{len(mails)} (limit 25)*\n\n"
    for x in mails[:25]: n="NEW " if x.get("mail_id") not in s["seen"] else ""; s["seen"].add(x.get("mail_id")); t+=f"{n}`{x.get('mail_id')}` {x.get('mail_from','?')}\n{x.get('mail_subject','—')}\n\n"
