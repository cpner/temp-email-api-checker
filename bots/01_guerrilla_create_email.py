#!/usr/bin/env python3
"""
Guerrilla Mail — Create Email

Provider: Guerrilla Mail | Endpoint: get_email_address
Docs: guerrillamail.com/GuerrillaMailAPI.html
"""
import telebot, requests, random, string, time, os

BOT_TOKEN = os.environ.get("BOT_TOKEN_GCREATE", "YOUR_TOKEN")
bot = telebot.TeleBot(BOT_TOKEN)
BASE = "https://api.guerrillamail.com/ajax.php"
SVC = "Guerrilla Create"
sess = {}
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("New Email", callback_data="gc_new"))
    kb.add(types.InlineKeyboardButton("Inbox", callback_data="gc_inbox"))
    kb.add(types.InlineKeyboardButton("Info", callback_data="gc_info"))
    kb.add(types.InlineKeyboardButton("Help", callback_data="gc_help"))
    bot.send_message(m.chat.id, "*Guerrilla Mail — Create*\n\n\n/new — Create\n/inbox — Check\n/info — Info\n/help — Help", parse_mode="Markdown", reply_markup=kb)

@bot.message_handler(commands=["new"])
def cmd_new(m):
    c=m.chat.id; s=gs(c)
    r=g(p={{"f":"get_email_address","ip":"127.0.0.1","agent":"Mozilla"}})
    if "email_addr" in r: s.update(addr=r["email_addr"],token=r.get("sid_token"),seen=set()); bot.send_message(c,f"Created: `{r['email_addr']}`",parse_mode="Markdown")
...
def cmd_inbox(m):
    ...
    r=g(p={{"f":"check_email","sid_token":s["token"],"seq":0}})
    msgs=r.get("list",[])
...
def cmd_set(m):
    ...
    s=gs(m.chat.id); r=g(p={{"f":"set_email_user","sid_token":s.get("token",""),"email_user":p[1].strip()}})
    if "email_addr" in r: s["addr"]=r["email_addr"]; bot.send_message(m.chat.id,f"`{r['email_addr']}`",parse_mode="Markdown")

@bot.message_handler(commands=["info"])
def cmd_info(m): s=gs(m.chat.id); bot.send_message(m.chat.id,f"Email: {s.get('addr','—')}")
...
def cb_new(c,call):
    ...
    if "email_addr" in r: s.update(addr=r["email_addr"],token=r.get("sid_token"),seen=set()); bot.edit_message_text(f"Created: `{r['email_addr']}`",c,call.message.message_id,parse_mode="Markdown")
...
def cb_inbox(c,call):
    ...
    bot.edit_message_text(f"{len(msgs)} msgs:\n\n"+txt,c,call.message.message_id)

@bot.callback_query_handler(func=lambda c: c.data.startswith("gc_"))
def cb(call):
    c = call.message.chat.id
    a = call.data.replace("gc_","")
