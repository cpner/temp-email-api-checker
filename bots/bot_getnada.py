...
BOT_TOKEN = os.environ.get("BOT_TOKEN_GETNADA", "YOUR_TOKEN")
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("📧 Новая", callback_data="gn_new"))
    kb.add(types.InlineKeyboardButton("📥 Письма", callback_data="gn_inbox"))
    kb.add(types.InlineKeyboardButton("🔑 Ключ", callback_data="gn_key"))
    kb.add(types.InlineKeyboardButton("📋 Данные", callback_data="gn_info"))
    kb.add(types.InlineKeyboardButton("❓ Помощь", callback_data="gn_help"))
    text = (
        "📧 *GetNada Bot*\n"
        "Многодоменная\n\n"
        "/new — Создать почту\n"
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("gn_"))
def cb(call):
    cid = call.message.chat.id
    act = call.data.replace("gn_", "")
