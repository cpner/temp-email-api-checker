#!/usr/bin/env python3
"""
MailDrop Telegram Bot
API v2
API: https://api.maildrop.cc/v2
"""
import telebot
...
BOT_TOKEN = os.environ.get("BOT_TOKEN_MDRP", "YOUR_TOKEN")
bot = telebot.TeleBot(BOT_TOKEN)
BASE = "https://api.maildrop.cc/v2"
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("📧 Новая", callback_data="md_new"))
    kb.add(types.InlineKeyboardButton("📥 Письма", callback_data="md_inbox"))
    kb.add(types.InlineKeyboardButton("🔑 Ключ", callback_data="md_key"))
    kb.add(types.InlineKeyboardButton("📋 Данные", callback_data="md_info"))
    kb.add(types.InlineKeyboardButton("❓ Помощь", callback_data="md_help"))
    text = (
        "📧 *MailDrop Bot*\n"
        "API v2\n\n"
        "/new — Создать почту\n"
