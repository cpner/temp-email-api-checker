#!/usr/bin/env python3
"""
Mailtrap Telegram Bot
Тестовый SMTP
API: https://api.mailtrap.io
"""
import telebot
...
BOT_TOKEN = os.environ.get("BOT_TOKEN_MAILTRAP", "YOUR_TOKEN")
bot = telebot.TeleBot(BOT_TOKEN)
BASE = "https://api.mailtrap.io"
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("📧 Новая", callback_data="mtr_new"))
    kb.add(types.InlineKeyboardButton("📥 Письма", callback_data="mtr_inbox"))
    kb.add(types.InlineKeyboardButton("🔑 Ключ", callback_data="mtr_key"))
    kb.add(types.InlineKeyboardButton("📋 Данные", callback_data="mtr_info"))
    kb.add(types.InlineKeyboardButton("❓ Помощь", callback_data="mtr_help"))
    text = (
        "📧 *Mailtrap Bot*\n"
        "Тестовый SMTP\n\n"
        "/new — Создать почту\n"
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("mtr_"))
def cb(call):
    cid = call.message.chat.id
    act = call.data.replace("mtr_", "")
