#!/usr/bin/env python3
"""
MailSlurp — Empty Inbox

Provider: MailSlurp | Endpoint: inboxes?page=1
Docs: docs.mailslurp.com
"""
import telebot, requests, random, string, time, os

BOT_TOKEN = os.environ.get("BOT_TOKEN_MLSLURP_EMP", "YOUR_TOKEN")
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("New Email", callback_data="mle_new"))
    kb.add(types.InlineKeyboardButton("Inbox", callback_data="mle_inbox"))
    kb.add(types.InlineKeyboardButton("Info", callback_data="mle_info"))
    kb.add(types.InlineKeyboardButton("Help", callback_data="mle_help"))
    bot.send_message(m.chat.id, "*MailSlurp — Empty Inbox*\n\n\n/new — Create\n/inbox — Check\n/info — Info\n/help — Help", parse_mode="Markdown", reply_markup=kb)
...
def cmd_inbox(m):
    ...
    r=g("/inboxes?page=1&size=10",h={"x-api-key":s["key"]}); ibs=r.get("content",[]) if isinstance(r,dict) else []
    if not ibs: return bot.send_message(m.chat.id,"No inboxes.")
    t=f"*{len(ibs)}*\n\n"
    for x in ibs[:10]: t+=f"`{x.get('id','?')[:12]}...` {x.get('emailAddress','?')}\n"
    bot.send_message(m.chat.id,t,parse_mode="Markdown")

@bot.message_handler(commands=["help"])
def cmd_help(m): bot.send_message(m.chat.id,"/key <KEY>\n/inbox")
@bot.callback_query_handler(func=lambda c: c.data.startswith("mle_"))
def cb(call):
    c = call.message.chat.id
    a = call.data.replace("mle_","")
