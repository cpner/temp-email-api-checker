#!/usr/bin/env python3
"""
SpamGourmet Telegram Bot
С завершением
API: https://www.spamgourmet.com/xmlapi.pl
"""
import telebot
...
BOT_TOKEN = os.environ.get("BOT_TOKEN_SPAMG", "YOUR_TOKEN")
bot = telebot.TeleBot(BOT_TOKEN)
BASE = "https://www.spamgourmet.com/xmlapi.pl"
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("📧 Новая", callback_data="sg_new"))
    kb.add(types.InlineKeyboardButton("📥 Письма", callback_data="sg_inbox"))
    kb.add(types.InlineKeyboardButton("🔑 Ключ", callback_data="sg_key"))
    kb.add(types.InlineKeyboardButton("📋 Данные", callback_data="sg_info"))
    kb.add(types.InlineKeyboardButton("❓ Помощь", callback_data="sg_help"))
    text = (
        "📧 *SpamGourmet Bot*\n"
        "С завершением\n\n"
        "/new — Создать почту\n"
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("sg_"))
def cb(call):
    cid = call.message.chat.id
    act = call.data.replace("sg_", "")
