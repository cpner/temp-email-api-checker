...
Provider: YOPmail | Endpoint: yopmail.com
Docs: yopmail.com
"""
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("New Email", callback_data="yp_new"))
    kb.add(types.InlineKeyboardButton("Inbox", callback_data="yp_inbox"))
    kb.add(types.InlineKeyboardButton("Info", callback_data="yp_info"))
    kb.add(types.InlineKeyboardButton("Help", callback_data="yp_help"))
    bot.send_message(m.chat.id, "*YOPmail*\n\n\n/new — Create\n/inbox — Check\n/info — Info\n/help — Help", parse_mode="Markdown", reply_markup=kb)
...
def cmd_domains(m):
    bot.send_message(m.chat.id,"*YOPmail Domains:*\n\n• yopmail.com\n• yopmail.fr\n• yopmail.gq\n• drdrb.com\n• c2.hu\n• 1chuan.com",parse_mode="Markdown")

@bot.message_handler(commands=["help"])
def cmd_help(m): bot.send_message(m.chat.id,"/check <name> — Check\n/domains — Domains")

def cb_new(c,call): bot.send_message(c,"/check <name>")
def cb_inbox(c,call): bot.send_message(c,"Visit yopmail.com")

@bot.callback_query_handler(func=lambda c: c.data.startswith("yp_"))
def cb(call):
    c = call.message.chat.id
    a = call.data.replace("yp_","")
