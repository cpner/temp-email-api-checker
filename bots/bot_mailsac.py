...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("📧 Новая", callback_data="ms_new"))
    kb.add(types.InlineKeyboardButton("📥 Письма", callback_data="ms_inbox"))
    kb.add(types.InlineKeyboardButton("🔑 Ключ", callback_data="ms_key"))
    kb.add(types.InlineKeyboardButton("📋 Данные", callback_data="ms_info"))
    kb.add(types.InlineKeyboardButton("❓ Помощь", callback_data="ms_help"))
    text = (
        "📧 *MailSac Bot*\n"
        "Про API\n\n"
        "/new — Создать почту\n"
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("ms_"))
def cb(call):
    cid = call.message.chat.id
    act = call.data.replace("ms_", "")
