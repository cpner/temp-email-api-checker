...
BOT_TOKEN = os.environ.get("BOT_TOKEN_DROPMAIL", "YOUR_TOKEN")
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("📧 Новая", callback_data="dm_new"))
    kb.add(types.InlineKeyboardButton("📥 Письма", callback_data="dm_inbox"))
    kb.add(types.InlineKeyboardButton("🔑 Ключ", callback_data="dm_key"))
    kb.add(types.InlineKeyboardButton("📋 Данные", callback_data="dm_info"))
    kb.add(types.InlineKeyboardButton("❓ Помощь", callback_data="dm_help"))
    text = (
        "📧 *DropMail Bot*\n"
        "GraphQL API\n\n"
        "/new — Создать почту\n"
