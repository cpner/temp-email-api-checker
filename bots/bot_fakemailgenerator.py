#!/usr/bin/env python3
"""
FakeMailGenerator Telegram Bot
Генератор
API: https://www.fakemailgenerator.com
"""
import telebot
...
BOT_TOKEN = os.environ.get("BOT_TOKEN_FAKEMAIL", "YOUR_TOKEN")
bot = telebot.TeleBot(BOT_TOKEN)
BASE = "https://www.fakemailgenerator.com"
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("📧 Новая", callback_data="fmg_new"))
    kb.add(types.InlineKeyboardButton("📥 Письма", callback_data="fmg_inbox"))
    kb.add(types.InlineKeyboardButton("🔑 Ключ", callback_data="fmg_key"))
    kb.add(types.InlineKeyboardButton("📋 Данные", callback_data="fmg_info"))
    kb.add(types.InlineKeyboardButton("❓ Помощь", callback_data="fmg_help"))
    text = (
        "📧 *FakeMailGenerator Bot*\n"
        "Генератор\n\n"
        "/new — Создать почту\n"
...
def cmd_new(m):
    ...
    addr = f"{rnd}@fakemailgenerator.com"
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("fmg_"))
def cb(call):
    cid = call.message.chat.id
    act = call.data.replace("fmg_", "")
...
if __name__ == "__main__":
    print("[FakeMailGenerator Bot] Running...")
