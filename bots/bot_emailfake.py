#!/usr/bin/env python3
"""
Telegram-бот: Временная почта через EmailFake API
Простая проверка почтовых ящиков
"""
...
def cmd_start(msg):
    ...
    markup.add(
        types.InlineKeyboardButton("📧 Установить почту", callback_data="ef_set"),
        types.InlineKeyboardButton("📥 Проверить", callback_data="ef_inbox"),
        types.InlineKeyboardButton("📋 Данные", callback_data="ef_info"),
        types.InlineKeyboardButton("❓ Помощь", callback_data="ef_help")
    )
    text = (
        "🎭 *EmailFake Bot*\n\n"
        "Проверка временных почтовых ящиков\n\n"
