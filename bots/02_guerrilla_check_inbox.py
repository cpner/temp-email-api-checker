#!/usr/bin/env python3
"""
Guerrilla Mail — Check Inbox

Provider: Guerrilla Mail | Endpoint: check_email
Docs: guerrillamail.com/GuerrillaMailAPI.html
"""
import telebot, requests, random, string, time, os

BOT_TOKEN = os.environ.get("BOT_TOKEN_GINBOX", "YOUR_TOKEN")
...
def cmd_start(m):
    ...
    kb.add(types.InlineKeyboardButton("New Email", callback_data="gi_new"))
    kb.add(types.InlineKeyboardButton("Inbox", callback_data="gi_inbox"))
    kb.add(types.InlineKeyboardButton("Info", callback_data="gi_info"))
    kb.add(types.InlineKeyboardButton("Help", callback_data="gi_help"))
    bot.send_message(m.chat.id, "*Guerrilla Mail — Inbox*\n\n\n/new — Create\n/inbox — Check\n/info — Info\n/help — Help", parse_mode="Markdown", reply_markup=kb)
...
def cmd_inbox(m):
    ...
    t=f"*{len(msgs)}*\n\n"
    for x in msgs[:20]: n="NEW " if x.get("mail_id") not in s["seen"] else ""; s["seen"].add(x.get("mail_id")); t+=f"{n}`{x.get('mail_id')}` {x.get('mail_from','?')}\n{x.get('mail_subject','—')}\n\n"
    bot.send_message(c,t,parse_mode="Markdown")

@bot.message_handler(commands=["info"])
def cmd_info(m): s=gs(m.chat.id); bot.send_message(m.chat.id,f"{s.get('addr','—')} | Read: {len(s.get('seen',set()))}")
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("gi_"))
def cb(call):
    c = call.message.chat.id
    a = call.data.replace("gi_","")
