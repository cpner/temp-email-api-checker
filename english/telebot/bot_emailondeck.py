...
Provider: EmailOnDeck | API: https://api.emailondeck.com/v1
...
BOT_TOKEN: str = os.environ.get("BOT_TOKEN_EMAILONDECK", "YOUR_BOT_TOKEN")
BASE_URL: str = "https://api.emailondeck.com/v1"
SERVICE_NAME: str = "EmailOnDeck"
...
if not BOT_TOKEN or BOT_TOKEN == "YOUR_BOT_TOKEN":
    logger.error("BOT_TOKEN not set! Set environment variable BOT_TOKEN_EMAILONDECK")
    sys.exit(1)
...
def cmd_info(message: types.Message) -> None:
    bot.send_message(message.chat.id, f"*EmailOnDeck*\n\n🌐 https://api.emailondeck.com/v1\n\nVisit the website to use this service.")
...
def callback_handler(call: types.CallbackQuery) -> None:
    ...
    try:
        if action == "new":
        bot.send_message(cid, f"Visit https://api.emailondeck.com/v1 to create an email.")
        elif action == "inbox":
        bot.send_message(cid, f"Visit https://api.emailondeck.com/v1 to check your inbox.")
        elif action == "info":
            s = get_session(cid)
            text = (
                f"*Session Info*\n\n"
