...
if not BOT_TOKEN or BOT_TOKEN == "YOUR_BOT_TOKEN":
    logger.error("BOT_TOKEN not set! Set environment variable BOT_TOKEN_EMAILFAKE")
    sys.exit(1)
...
def callback_handler(call: types.CallbackQuery) -> None:
    ...
    try:
        if action == "new":
        bot.send_message(cid, f"Visit https://emailfake.com/api/v1 to create an email.")
        elif action == "inbox":
        bot.send_message(cid, f"Visit https://emailfake.com/api/v1 to check your inbox.")
        elif action == "info":
            s = get_session(cid)
            text = (
                f"*Session Info*\n\n"
