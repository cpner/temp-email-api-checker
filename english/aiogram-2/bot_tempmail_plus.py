...
def make_kb():
    ...
    kb.add(InlineKeyboardButton("Set Email", callback_data="new"), InlineKeyboardButton("Inbox", callback_data="inbox"))
    kb.add(InlineKeyboardButton("Info", callback_data="info"), InlineKeyboardButton("Source", callback_data="source"))
    kb.add(InlineKeyboardButton("Help", callback_data="help"))
    return kb

@dp.message_handler(commands=["start", "menu"])
async def cmd_start(m):
    await m.answer( "TempMail.plus Bot\nMonitor inbox for any email\n\nHow to use:\n1. /set email@domain.com\n2. /inbox to check", reply_markup=make_kb())
...
async def cb(call):
    ...
    try:
        if call.data == "new":
            ...
            await bot.edit_message_text("Source: https://github.com/cpner/temp-email-api-checker/blob/main/english/aiogram-2/bot_tempmail_plus.py/blob/main/english/aiogram-2/bot_tempmail_plus.py", c, call.message.message_id, reply_markup=make_kb())
        elif call.data == "help":
